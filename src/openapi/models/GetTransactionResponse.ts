 ...
 * @export
 */
export type GetTransactionResponse = { type: 'black_box_signature' } & BlackBoxSignature | { type: 'cosmos_transaction' } & CosmosTransaction | { type: 'evm_message' } & EvmMessage | { type: 'evm_transaction' } & EvmTransaction | { type: 'solana_message' } & SolanaMessage | { type: 'solana_transaction' } & SolanaTransaction | { type: 'utxo_transaction' } & UtxoTransaction;
...
export function GetTransactionResponseToJSON(value?: GetTransactionResponse | null): any {
    ...
    switch (value['type']) {
        ...
        default:
            throw new Error(`No variant of GetTransactionResponse exists with 'type=${value['type']}'`);
    }
