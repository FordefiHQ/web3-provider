/* tslint:disable */
/* eslint-disable */
/**
 * Fordefi API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.2.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { AptosMessageState } from './AptosMessageState';
import {
    AptosMessageStateFromJSON,
    AptosMessageStateFromJSONTyped,
    AptosMessageStateToJSON,
} from './AptosMessageState';
import type { PricedAsset } from './PricedAsset';
import {
    PricedAssetFromJSON,
    PricedAssetFromJSONTyped,
    PricedAssetToJSON,
} from './PricedAsset';
import type { StateChangeReason } from './StateChangeReason';
import {
    StateChangeReasonFromJSON,
    StateChangeReasonFromJSONTyped,
    StateChangeReasonToJSON,
} from './StateChangeReason';

/**
 * 
 * @export
 * @interface AptosMessageStateChange
 */
export interface AptosMessageStateChange {
    /**
     * 
     * @type {Date}
     * @memberof AptosMessageStateChange
     */
    changedAt: Date;
    /**
     * 
     * @type {StateChangeReason}
     * @memberof AptosMessageStateChange
     */
    reason?: StateChangeReason;
    /**
     * 
     * @type {Array<PricedAsset>}
     * @memberof AptosMessageStateChange
     * @deprecated
     */
    assetPrices: Array<PricedAsset>;
    /**
     * 
     * @type {AptosMessageState}
     * @memberof AptosMessageStateChange
     */
    previousState?: AptosMessageState;
    /**
     * 
     * @type {AptosMessageState}
     * @memberof AptosMessageStateChange
     */
    newState: AptosMessageState;
}

/**
 * Check if a given object implements the AptosMessageStateChange interface.
 */
export function instanceOfAptosMessageStateChange(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "changedAt" in value;
    isInstance = isInstance && "assetPrices" in value;
    isInstance = isInstance && "newState" in value;

    return isInstance;
}

export function AptosMessageStateChangeFromJSON(json: any): AptosMessageStateChange {
    return AptosMessageStateChangeFromJSONTyped(json, false);
}

export function AptosMessageStateChangeFromJSONTyped(json: any, ignoreDiscriminator: boolean): AptosMessageStateChange {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'changedAt': (new Date(json['changed_at'])),
        'reason': !exists(json, 'reason') ? undefined : StateChangeReasonFromJSON(json['reason']),
        'assetPrices': ((json['asset_prices'] as Array<any>).map(PricedAssetFromJSON)),
        'previousState': !exists(json, 'previous_state') ? undefined : AptosMessageStateFromJSON(json['previous_state']),
        'newState': AptosMessageStateFromJSON(json['new_state']),
    };
}

export function AptosMessageStateChangeToJSON(value?: AptosMessageStateChange | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'changed_at': (value.changedAt.toISOString()),
        'reason': StateChangeReasonToJSON(value.reason),
        'asset_prices': ((value.assetPrices as Array<any>).map(PricedAssetToJSON)),
        'previous_state': AptosMessageStateToJSON(value.previousState),
        'new_state': AptosMessageStateToJSON(value.newState),
    };
}

