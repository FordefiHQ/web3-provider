/* tslint:disable */
/* eslint-disable */
/**
 * Fordefi API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.2.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import {
    PredictedCosmosTransaction,
    instanceOfPredictedCosmosTransaction,
    PredictedCosmosTransactionFromJSON,
    PredictedCosmosTransactionFromJSONTyped,
    PredictedCosmosTransactionToJSON,
} from './PredictedCosmosTransaction';
import {
    PredictedEvmMessage,
    instanceOfPredictedEvmMessage,
    PredictedEvmMessageFromJSON,
    PredictedEvmMessageFromJSONTyped,
    PredictedEvmMessageToJSON,
} from './PredictedEvmMessage';
import {
    PredictedEvmTransaction,
    instanceOfPredictedEvmTransaction,
    PredictedEvmTransactionFromJSON,
    PredictedEvmTransactionFromJSONTyped,
    PredictedEvmTransactionToJSON,
} from './PredictedEvmTransaction';
import {
    PredictedSolanaMessage,
    instanceOfPredictedSolanaMessage,
    PredictedSolanaMessageFromJSON,
    PredictedSolanaMessageFromJSONTyped,
    PredictedSolanaMessageToJSON,
} from './PredictedSolanaMessage';
import {
    PredictedSolanaTransaction,
    instanceOfPredictedSolanaTransaction,
    PredictedSolanaTransactionFromJSON,
    PredictedSolanaTransactionFromJSONTyped,
    PredictedSolanaTransactionToJSON,
} from './PredictedSolanaTransaction';
import {
    PredictedUtxoTransaction,
    instanceOfPredictedUtxoTransaction,
    PredictedUtxoTransactionFromJSON,
    PredictedUtxoTransactionFromJSONTyped,
    PredictedUtxoTransactionToJSON,
} from './PredictedUtxoTransaction';

/**
 * @type BulkPredictTransactionResponse
 * 
 * @export
 */
export type BulkPredictTransactionResponse = { type: 'cosmos_transaction' } & PredictedCosmosTransaction | { type: 'evm_message' } & PredictedEvmMessage | { type: 'evm_transaction' } & PredictedEvmTransaction | { type: 'solana_message' } & PredictedSolanaMessage | { type: 'solana_transaction' } & PredictedSolanaTransaction | { type: 'utxo_transaction' } & PredictedUtxoTransaction;

export function BulkPredictTransactionResponseFromJSON(json: any): BulkPredictTransactionResponse {
    return BulkPredictTransactionResponseFromJSONTyped(json, false);
}

export function BulkPredictTransactionResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): BulkPredictTransactionResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    switch (json['type']) {
        case 'cosmos_transaction':
            return {...PredictedCosmosTransactionFromJSONTyped(json, true), type: 'cosmos_transaction'};
        case 'evm_message':
            return {...PredictedEvmMessageFromJSONTyped(json, true), type: 'evm_message'};
        case 'evm_transaction':
            return {...PredictedEvmTransactionFromJSONTyped(json, true), type: 'evm_transaction'};
        case 'solana_message':
            return {...PredictedSolanaMessageFromJSONTyped(json, true), type: 'solana_message'};
        case 'solana_transaction':
            return {...PredictedSolanaTransactionFromJSONTyped(json, true), type: 'solana_transaction'};
        case 'utxo_transaction':
            return {...PredictedUtxoTransactionFromJSONTyped(json, true), type: 'utxo_transaction'};
        default:
            throw new Error(`No variant of BulkPredictTransactionResponse exists with 'type=${json['type']}'`);
    }
}

export function BulkPredictTransactionResponseToJSON(value?: BulkPredictTransactionResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    switch (value['type']) {
        case 'cosmos_transaction':
            return PredictedCosmosTransactionToJSON(value);
        case 'evm_message':
            return PredictedEvmMessageToJSON(value);
        case 'evm_transaction':
            return PredictedEvmTransactionToJSON(value);
        case 'solana_message':
            return PredictedSolanaMessageToJSON(value);
        case 'solana_transaction':
            return PredictedSolanaTransactionToJSON(value);
        case 'utxo_transaction':
            return PredictedUtxoTransactionToJSON(value);
        default:
            throw new Error(`No variant of BulkPredictTransactionResponse exists with 'type=${value['type']}'`);
    }

}

